...
inline bool Vector2::in_bounds(const Vector2& Bounds) const noexcept {
...
inline float Vector2::distance_squared(const Vector2& v1, const Vector2& v2) noexcept {
	using namespace DirectX;
	const XMVECTOR x1 = XMLoadFloat2(&v1);
	const XMVECTOR x2 = XMLoadFloat2(&v2);
	const XMVECTOR V = XMVectorSubtract(x2, x1);
...
inline void Vector2::smooth_step(const Vector2& v1, const Vector2& v2, float t, Vector2& result) noexcept {
	using namespace DirectX;
...
inline Vector2 Vector2::smooth_step(const Vector2& v1, const Vector2& v2, float t) noexcept {
	using namespace DirectX;
	t = (t > 1.0f) ? 1.0f : ((t < 0.0f) ? 0.0f : t); // Clamp value to 0 to 1
	t = t * t * (3.f - 2.f * t);
	const XMVECTOR x1 = XMLoadFloat2(&v1);
...
inline void Vector2::catmull_rom(const Vector2& v1, const Vector2& v2, const Vector2& v3, const Vector2& v4, float t, Vector2& result) noexcept {
	using namespace DirectX;
	const XMVECTOR x1 = XMLoadFloat2(&v1);
	const XMVECTOR x2 = XMLoadFloat2(&v2);
	const XMVECTOR x3 = XMLoadFloat2(&v3);
...
inline Vector2 Vector2::catmull_rom(const Vector2& v1, const Vector2& v2, const Vector2& v3, const Vector2& v4, float t) noexcept {
	using namespace DirectX;
	const XMVECTOR x1 = XMLoadFloat2(&v1);
	const XMVECTOR x2 = XMLoadFloat2(&v2);
	const XMVECTOR x3 = XMLoadFloat2(&v3);
...
inline void Vector2::transform_normal(const Vector2& v, const Matrix& m, Vector2& result) noexcept {
	using namespace DirectX;
	const XMVECTOR v1 = XMLoadFloat2(&v);
	const XMMATRIX M = XMLoadFloat4x4(&m);
...
inline Vector2 Vector2::transform_normal(const Vector2& v, const Matrix& m) noexcept {
	using namespace DirectX;
	const XMVECTOR v1 = XMLoadFloat2(&v);
	const XMMATRIX M = XMLoadFloat4x4(&m);
	const XMVECTOR X = XMVector2TransformNormal(v1, M);
...
_Use_decl_annotations_ inline void Vector2::transform_normal(const Vector2* varray, size_t count, const Matrix& m, Vector2* resultArray) noexcept {
	using namespace DirectX;
	const XMMATRIX M = XMLoadFloat4x4(&m);
...
inline bool Vector3::in_bounds(const Vector3& Bounds) const noexcept {
	using namespace DirectX;
...
inline float Vector3::distance_squared(const Vector3& v1, const Vector3& v2) noexcept {
	using namespace DirectX;
	const XMVECTOR x1 = XMLoadFloat3(&v1);
	const XMVECTOR x2 = XMLoadFloat3(&v2);
	const XMVECTOR V = XMVectorSubtract(x2, x1);
...
inline void Vector3::smooth_step(const Vector3& v1, const Vector3& v2, float t, Vector3& result) noexcept {
	using namespace DirectX;
	t = (t > 1.0f) ? 1.0f : ((t < 0.0f) ? 0.0f : t); // Clamp value to 0 to 1
	t = t * t * (3.f - 2.f * t);
	const XMVECTOR x1 = XMLoadFloat3(&v1);
...
inline Vector3 Vector3::smooth_step(const Vector3& v1, const Vector3& v2, float t) noexcept {
	using namespace DirectX;
	t = (t > 1.0f) ? 1.0f : ((t < 0.0f) ? 0.0f : t); // Clamp value to 0 to 1
	t = t * t * (3.f - 2.f * t);
	const XMVECTOR x1 = XMLoadFloat3(&v1);
...
inline void Vector3::catmull_rom(const Vector3& v1, const Vector3& v2, const Vector3& v3, const Vector3& v4, float t, Vector3& result) noexcept {
	using namespace DirectX;
	const XMVECTOR x1 = XMLoadFloat3(&v1);
	const XMVECTOR x2 = XMLoadFloat3(&v2);
	const XMVECTOR x3 = XMLoadFloat3(&v3);
...
inline Vector3 Vector3::catmull_rom(const Vector3& v1, const Vector3& v2, const Vector3& v3, const Vector3& v4, float t) noexcept {
	using namespace DirectX;
	const XMVECTOR x1 = XMLoadFloat3(&v1);
	const XMVECTOR x2 = XMLoadFloat3(&v2);
	const XMVECTOR x3 = XMLoadFloat3(&v3);
...
inline void Vector3::transform_normal(const Vector3& v, const Matrix& m, Vector3& result) noexcept {
	using namespace DirectX;
	const XMVECTOR v1 = XMLoadFloat3(&v);
	const XMMATRIX M = XMLoadFloat4x4(&m);
...
inline Vector3 Vector3::transform_normal(const Vector3& v, const Matrix& m) noexcept {
	using namespace DirectX;
	const XMVECTOR v1 = XMLoadFloat3(&v);
	const XMMATRIX M = XMLoadFloat4x4(&m);
	const XMVECTOR X = XMVector3TransformNormal(v1, M);
...
_Use_decl_annotations_ inline void Vector3::transform_normal(const Vector3* varray, size_t count, const Matrix& m, Vector3* resultArray) noexcept {
	using namespace DirectX;
	const XMMATRIX M = XMLoadFloat4x4(&m);
...
inline bool Vector4::in_bounds(const Vector4& Bounds) const noexcept {
	using namespace DirectX;
	const XMVECTOR v1 = XMLoadFloat4(this);
...
inline float Vector4::length_squared() const noexcept {
	using namespace DirectX;
	const XMVECTOR v1 = XMLoadFloat4(this);
...
inline float Vector4::distance_squared(const Vector4& v1, const Vector4& v2) noexcept {
	using namespace DirectX;
	const XMVECTOR x1 = XMLoadFloat4(&v1);
	const XMVECTOR x2 = XMLoadFloat4(&v2);
	const XMVECTOR V = XMVectorSubtract(x2, x1);
...
inline void Vector4::smooth_step(const Vector4& v1, const Vector4& v2, float t, Vector4& result) noexcept {
	using namespace DirectX;
	t = (t > 1.0f) ? 1.0f : ((t < 0.0f) ? 0.0f : t); // Clamp value to 0 to 1
	t = t * t * (3.f - 2.f * t);
	const XMVECTOR x1 = XMLoadFloat4(&v1);
...
inline Vector4 Vector4::smooth_step(const Vector4& v1, const Vector4& v2, float t) noexcept {
	using namespace DirectX;
	t = (t > 1.0f) ? 1.0f : ((t < 0.0f) ? 0.0f : t); // Clamp value to 0 to 1
	t = t * t * (3.f - 2.f * t);
	const XMVECTOR x1 = XMLoadFloat4(&v1);
...
inline void Vector4::catmull_rom(const Vector4& v1, const Vector4& v2, const Vector4& v3, const Vector4& v4, float t, Vector4& result) noexcept {
	using namespace DirectX;
	const XMVECTOR x1 = XMLoadFloat4(&v1);
	const XMVECTOR x2 = XMLoadFloat4(&v2);
	const XMVECTOR x3 = XMLoadFloat4(&v3);
...
inline Vector4 Vector4::catmull_rom(const Vector4& v1, const Vector4& v2, const Vector4& v3, const Vector4& v4, float t) noexcept {
	using namespace DirectX;
	const XMVECTOR x1 = XMLoadFloat4(&v1);
	const XMVECTOR x2 = XMLoadFloat4(&v2);
	const XMVECTOR x3 = XMLoadFloat4(&v3);
...
inline Vector3 Matrix::to_euler() const noexcept {
...
_Use_decl_annotations_ inline Matrix Matrix::create_billboard(
...
_Use_decl_annotations_ inline Matrix Matrix::create_constrained_billboard(
		const Vector3& object,
		const Vector3& cameraPosition,
...
inline Matrix Matrix::create_translation(const Vector3& position) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_translation(float x, float y, float z) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_scale(const Vector3& scales) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_scale(float xs, float ys, float zs) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_scale(float scale) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_rotation_x(float radians) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_rotation_y(float radians) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_rotation_z(float radians) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_from_axis_angle(const Vector3& axis, float angle) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_perspective_field_of_view(float fov, float aspectRatio, float nearPlane, float farPlane) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_perspective(float width, float height, float nearPlane, float farPlane) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_perspective_off_center(float left, float right, float bottom, float top, float nearPlane, float farPlane) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_orthographic(float width, float height, float zNearPlane, float zFarPlane) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_orthographic_off_center(float left, float right, float bottom, float top, float zNearPlane, float zFarPlane) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_look_at(const Vector3& eye, const Vector3& target, const Vector3& up) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_world(const Vector3& position, const Vector3& forward, const Vector3& up) noexcept {
	using namespace DirectX;
...
inline Matrix Matrix::create_from_quaternion(const Quaternion& rotation) noexcept {
	using namespace DirectX;
...
inline Matrix Matrix::create_from_yaw_pitch_roll(float yaw, float pitch, float roll) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_from_yaw_pitch_roll(const Vector3& angles) noexcept {
	using namespace DirectX;
	Matrix R;
...
inline Matrix Matrix::create_shadow(const Vector3& lightDir, const Plane& plane) noexcept {
	using namespace DirectX;
...
inline Matrix Matrix::create_reflection(const Plane& plane) noexcept {
	using namespace DirectX;
	const XMVECTOR planev = XMLoadFloat4(&plane);
	Matrix R;
...
inline float Plane::dot_coordinate(const Vector3& position) const noexcept {
	using namespace DirectX;
	const XMVECTOR p = XMLoadFloat4(this);
...
inline float Plane::dot_normal(const Vector3& normal) const noexcept {
	using namespace DirectX;
	const XMVECTOR p = XMLoadFloat4(this);
...
inline void Quaternion::rotate_towards(const Quaternion& target, float maxAngle) noexcept {
	rotate_towards(target, maxAngle, *this);
}

inline Vector3 Quaternion::to_euler() const noexcept {
	const float xx = x * x;
	const float yy = y * y;
	const float zz = z * z;
...
inline Quaternion Quaternion::create_from_axis_angle(const Vector3& axis, float angle) noexcept {
	using namespace DirectX;
	const XMVECTOR a = XMLoadFloat3(&axis);
...
inline Quaternion Quaternion::create_from_yaw_pitch_roll(float yaw, float pitch, float roll) noexcept {
	using namespace DirectX;
	Quaternion R;
...
inline Quaternion Quaternion::create_from_yaw_pitch_roll(const Vector3& angles) noexcept {
	using namespace DirectX;
	Quaternion R;
...
inline Quaternion Quaternion::create_from_rotation_matrix(const Matrix& M) noexcept {
	using namespace DirectX;
	const XMMATRIX M0 = XMLoadFloat4x4(&M);
...
inline Quaternion Quaternion::from_to_rotation(const Vector3& fromDir, const Vector3& toDir) noexcept {
	Quaternion result;
	from_to_rotation(fromDir, toDir, result);
	return result;
}

inline Quaternion Quaternion::look_rotation(const Vector3& forward, const Vector3& up) noexcept {
	Quaternion result;
	look_rotation(forward, up, result);
	return result;
}
...
inline void Color::adjust_saturation(float sat) noexcept {
	using namespace DirectX;
	const XMVECTOR c = XMLoadFloat4(this);
	XMStoreFloat4(this, XMColorAdjustSaturation(c, sat));
}

inline void Color::adjust_saturation(float sat, Color& result) const noexcept {
	using namespace DirectX;
	const XMVECTOR c = XMLoadFloat4(this);
	XMStoreFloat4(&result, XMColorAdjustSaturation(c, sat));
}

inline void Color::adjust_contrast(float contrast) noexcept {
	using namespace DirectX;
	const XMVECTOR c = XMLoadFloat4(this);
	XMStoreFloat4(this, XMColorAdjustContrast(c, contrast));
}

inline void Color::adjust_contrast(float contrast, Color& result) const noexcept {
	using namespace DirectX;
	const XMVECTOR c = XMLoadFloat4(this);
...
inline float Viewport::aspect_ratio() const noexcept {
...
inline Vector3 Viewport::project(const Vector3& p, const Matrix& proj, const Matrix& view, const Matrix& world) const noexcept {
	...
	v = XMVector3Project(v, x, y, width, height, min_depth, max_depth, projection, view, world);
	Vector3 result;
...
inline Vector3 Viewport::unproject(const Vector3& p, const Matrix& proj, const Matrix& view, const Matrix& world) const noexcept {
	...
	XMVECTOR v = XMLoadFloat3(&p);
	const XMMATRIX projection = XMLoadFloat4x4(&proj);
	v = XMVector3Unproject(v, x, y, width, height, min_depth, max_depth, projection, view, world);
	Vector3 result;
	XMStoreFloat3(&result, v);
	return result;
}
